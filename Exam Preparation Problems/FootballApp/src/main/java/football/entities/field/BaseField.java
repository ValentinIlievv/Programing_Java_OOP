...
public abstract class BaseField implements Field{
    ...
    public String getInfo() {
        ...
        String type = null;
        if (this.getClass().getSimpleName().equals("ArtificialTurf")){
            type = "ArtificialTurf";
        }
        else if (this.getClass().getSimpleName().equals("NaturalGrass")){
            type = "NaturalGrass";
        }
        sb.append(this.name).append(" (").append(type).append("):").append(System.lineSeparator());
        sb.append("Player: ");
        if (this.players.size() == 0){
            sb.append("none").append(System.lineSeparator());
        }
        else {
            this.players.forEach(player -> sb.append(player.getName()).append(" "));
        }
